...
TEST_F(TestMatrix, IntegralDeterminant5x5) {
    matrix_it m{
        {1, 2, 0, 2, 1}, {2, 1, 1, 1, 2}, {1, 3, 2, 1, 2}, {3, 2, 1, 1, 2}, {2, 2, 1, 2, 1}};
    ASSERT_TRUE(m.determinant() == -12);

    m = matrix_it{
        {1, 2, 3, 4, 5}, {0, 2, 3, 4, 5}, {0, 0, 3, 4, 5}, {0, 0, 0, 4, 5}, {0, 0, 0, 0, 5},
    };
    ASSERT_TRUE(m.determinant() == 120);

    m = matrix_it{
        {1, 2, 0, 4, 5},     {6, 7, 0, 9, 10},    {11, 12, 0, 14, 15},
        {16, 17, 0, 19, 20}, {21, 22, 0, 24, 25},
    };
    ASSERT_TRUE(m.determinant() == 0);
...
TEST_F(TestMatrix, FloatingDeterminant5x5) {
    ...
    m = matrix_ft{
        {1.1, 2, 3, 4, 5}, {0, 2.2, 3, 4, 5}, {0, 0, 3.3, 4, 5},
        {0, 0, 0, 4.4, 5}, {0, 0, 0, 0, 5.5},
    };
    ASSERT_TRUE(feq(m.determinant(), 193.2612));

    m = matrix_ft{{1.2, 2.1, 0, 2.5, 1.3},
                  {2.4, 1.1, 0, 1.4, 2.7},
                  {1.8, 3.1, 0, 1.3, 2.2},
